...
for i = 1:NUMBER_REPLICATE
    ...
    if any(i==printmat)
        % output
        printorder = A.data(j,1) + (printII-1)*number_atom;
        fprintf(fileID,formatSpec,printII,molecule_name,atom,printorder,pos);
    end
...
for i = 1:size(pair,1)
    ...
    for j = 1: nipa_chain
        ...
        connect_pair(connect_num,3:4) = [printII, 8 + (IIN-1)*number_atom1 + USEFUL_BIS*number_atom];
        connect_num = connect_num+1;
        connect_pair(connect_num,1:2) = [printII, 9 + (IIN-1)*number_atom1 + USEFUL_BIS*number_atom];
        ...
        % order of atom
        orderN = k + (IIN-1)*number_atom1 + USEFUL_BIS*number_atom;

        % output
        fprintf(fileID,formatSpec,printII,molecule_name1,atomN,orderN,new_pos(k,:));
        end
    end
